...
def register():
    ...
    if (test):
        return jsonify(message='The email is already exist'), 409
    else:
...
def add_planet():
    ...
    db.session.commit()
    return jsonify(message='New planet is created'), 201
...
def remove_planet(planet_id: int):
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message='Delete planet succes')
    else:
        return jsonify(message='No planet found with the id', planet_id=planet_id)
...
def remove_user(user_id: int):
    ...
    if user:
        ...
        return jsonify(message='No user found with the id', id=user_id)
